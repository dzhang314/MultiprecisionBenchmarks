...
#pragma omp parallel for
    for (std::size_t i = 0; i < n; ++i) {
        ...
        std::size_t j = 0;
        for (; j + 8 <= n; j += 8) {
            const v8f64x1 x(_mm512_loadu_pd(x0 + j));
            const v8f64x1 A(_mm512_loadu_pd(A0 + i * n + j));
            vector_sum += A * x;
        }
        for (; j < n; ++j) {
            const f64x1 x(x0[j]);
...
#pragma omp parallel for
    for (std::size_t i = 0; i < n; ++i) {
        ...
        std::size_t j = 0;
        for (; j + 8 <= n; j += 8) {
            const v8f64x2 x(_mm512_loadu_pd(x0 + j), _mm512_loadu_pd(x1 + j));
            const v8f64x2 A(_mm512_loadu_pd(A0 + i * n + j),
                            _mm512_loadu_pd(A1 + i * n + j));
            vector_sum += A * x;
        }
        for (; j < n; ++j) {
...
#pragma omp parallel for
    for (std::size_t i = 0; i < n; ++i) {
        ...
        std::size_t j = 0;
        for (; j + 8 <= n; j += 8) {
            const v8f64x3 x(_mm512_loadu_pd(x0 + j), _mm512_loadu_pd(x1 + j),
                            _mm512_loadu_pd(x2 + j));
            const v8f64x3 A(_mm512_loadu_pd(A0 + i * n + j),
                            _mm512_loadu_pd(A1 + i * n + j),
                            _mm512_loadu_pd(A2 + i * n + j));
            vector_sum += A * x;
        }
        for (; j < n; ++j) {
...
#pragma omp parallel for
    for (std::size_t i = 0; i < n; ++i) {
        ...
        std::size_t j = 0;
        for (; j + 8 <= n; j += 8) {
            const v8f64x4 x(_mm512_loadu_pd(x0 + j), _mm512_loadu_pd(x1 + j),
                            _mm512_loadu_pd(x2 + j), _mm512_loadu_pd(x3 + j));
            const v8f64x4 A(_mm512_loadu_pd(A0 + i * n + j),
                            _mm512_loadu_pd(A1 + i * n + j),
                            _mm512_loadu_pd(A2 + i * n + j),
                            _mm512_loadu_pd(A3 + i * n + j));
            vector_sum += A * x;
        }
        for (; j < n; ++j) {
