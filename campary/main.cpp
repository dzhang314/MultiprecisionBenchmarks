...
static void axpy(multi_prec<1> *y, multi_prec<1> a, const multi_prec<1> *x,
                 std::size_t n) {
...
static void axpy_bench_1(benchmark::State &bs) {
    ...
    multi_prec<1> *const y =
        static_cast<multi_prec<1> *>(std::malloc(n * sizeof(multi_prec<1>)));

    const multi_prec<1> a = static_cast<multi_prec<1>>(0.5);

    multi_prec<1> *const x =
        static_cast<multi_prec<1> *>(std::malloc(n * sizeof(multi_prec<1>)));
